...
void preallocate_memory(uint32_t *input,size_t height, size_t width, uint8_t bins[HISTO_HEIGHT*HISTO_WIDTH], uint32_t * temp_input)
{
  ...
  d_input = AllocateDevice(height*width*sizeof(uint32_t));
  HostToDevice(d_input, input, height*width*sizeof(uint32_t));
//    cudaMemcpy(d_input,input, height*width*sizeof(uint32_t), cudaMemcpyHostToDevice);
